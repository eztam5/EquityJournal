...
Desktop PDF and image bytes default to the application's data directory under `attachments/`. PDFs use `securities/{security-id}/`, while note images use `securities/{security-id}/images/` or `topics/{topic-id}/images/`. The Documents section in Settings can select another absolute attachment root. Saving a changed location copies the managed `securities`, `topics`, and `favicons` trees to the new root before switching the configuration, then removes the old managed trees. The setting is stored separately from the SQLite database configuration in `document-storage.json`.

Security link favicons are fetched when a URL field loses focus or settings are saved. Discovery checks the domain home page for declared icons, then `/favicon.ico`; unavailable or unsupported images retain the generic link icon. ICO, PNG, JPEG, GIF, and WebP files are stored under `favicons/` in the same managed attachment root (IndexedDB in browser development, subject to site CORS). Migration `010_security_link_favicons.sql` adds the relative `favicon_path` to each link template. Backups should include both the SQLite database and the entire managed attachment root. The security details page uses icon-only buttons with the resolved URL as the hover title and the link name as the accessible label.
