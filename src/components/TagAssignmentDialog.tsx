...
export function TagAssignmentDialog({securityId,onClose,onSaved}:{securityId:string;onClose():void;onSaved():void}){
  ...
  const save=async()=>{try{await app.repository.setAssignedTags(securityId,[...selected]);app.taxonomies.forEach((taxonomy)=>notifyTaxonomyTreeChanged(taxonomy.id));onSaved();onClose()}catch(r){setError(r instanceof Error?r.message:String(r))}}
