...
import { Button, Menu, MenuItem, PopoverNext, showContextMenu } from '@blueprintjs/core'
...
export function Sidebar({ onNewSecurity, onNewWatchlist, onNewTaxonomy }: { onNewSecurity():void;onNewWatchlist():void;onNewTaxonomy():void }) {
  ...
  const renderTaxonomyNode=(node:TaxonomyTreeModelNode,depth:number):React.ReactNode=>{
    ...
    return <Fragment key={node.id}><div className="taxonomy-sidebar-row" style={{'--taxonomy-depth':depth} as React.CSSProperties}>{hasChildren?<Button variant="minimal" size="small" className="taxonomy-sidebar-toggle" icon={isExpanded?'chevron-down':'chevron-right'} aria-label={`${isExpanded?'Collapse':'Expand'} ${node.tag.name}`} onClick={()=>toggleTag(node.id)}/>:<span className="taxonomy-sidebar-toggle-spacer"/>}<Button fill alignText="start" variant="minimal" className="nav-item taxonomy-sidebar-label" icon={<i className="nav-marker" style={{background:node.tag.color}}/>} text={node.tag.name} onClick={()=>hasChildren&&toggleTag(node.id)}/></div>{isExpanded&&node.children.map((child)=>renderTaxonomyNode(child,depth+1))}</Fragment>
  }
  return <><aside className="sidebar">
    ...
    <div className="sidebar-section"><SectionHeader title="Taxonomies" onAdd={onNewTaxonomy}/>{app.taxonomies.map((item)=>{const key=taxonomyNodeKey(item.id),isExpanded=expandedTaxonomyNodes.has(key),tree=taxonomyTrees[item.id],isLoading=loadingTaxonomies.has(item.id),hasChildren=tree===undefined||tree.length>0;return <Fragment key={item.id}><div className="taxonomy-sidebar-row taxonomy-sidebar-root">{hasChildren?<Button variant="minimal" size="small" className="taxonomy-sidebar-toggle" icon={isExpanded?'chevron-down':'chevron-right'} loading={isLoading} aria-label={`${isExpanded?'Collapse':'Expand'} ${item.name}`} onClick={()=>void toggleTaxonomy(item.id)}/>:<span className="taxonomy-sidebar-toggle-spacer"/>}<Button fill alignText="start" variant="minimal" className={`nav-item taxonomy-sidebar-label ${app.view.type==='taxonomy'&&app.view.id===item.id?'active':''}`} onClick={()=>app.setView({type:'taxonomy',id:item.id})} onContextMenu={(event)=>openDeleteMenu(event,()=>setDeletingTaxonomy(item))} icon={<i className="nav-marker" style={{background:item.color}}/>} text={item.name}/></div>{isExpanded&&tree?.map((node)=>renderTaxonomyNode(node,1))}{isExpanded&&taxonomyTreeErrors[item.id]&&<div className="taxonomy-sidebar-error">Could not load taxonomy.</div>}</Fragment>})}</div>
