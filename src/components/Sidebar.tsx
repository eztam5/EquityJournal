...
export function Sidebar({ onNewSecurity, onNewWatchlist, onNewTaxonomy, onNewTopic }: { onNewSecurity():void;onNewWatchlist():void;onNewTaxonomy():void;onNewTopic():void }) {
  ...
  const openAllSecuritiesMenu=(event:MouseEvent)=>{event.preventDefault();showContextMenu({targetOffset:{left:event.clientX,top:event.clientY},isDarkTheme:document.documentElement.classList.contains('bp6-dark'),content:<Menu><MenuItem icon="add" text="New security" onClick={onNewSecurity}/><MenuItem icon="refresh" text="Update quotes" onClick={()=>void app.updateQuotes()}/></Menu>})}
  const openWatchlistMenu=(event:MouseEvent,watchlist:Watchlist,index:number)=>{event.preventDefault();showContextMenu({targetOffset:{left:event.clientX,top:event.clientY},isDarkTheme:document.documentElement.classList.contains('bp6-dark'),content:<Menu><MenuItem icon="edit" text="Rename" onClick={()=>setRenamingWatchlist(watchlist)}/><MenuItem icon="refresh" text="Update quotes" onClick={()=>void app.updateQuotes(watchlist.id)}/><MenuItem icon="arrow-up" text="Move up" disabled={index===0} onClick={()=>app.moveWatchlist(watchlist.id,-1)}/><MenuItem icon="arrow-down" text="Move down" disabled={index===app.watchlists.length-1} onClick={()=>app.moveWatchlist(watchlist.id,1)}/><MenuDivider/><MenuItem icon="trash" intent="danger" text="Delete" onClick={()=>setDeletingWatchlist(watchlist)}/></Menu>})}
  ...
  const renderTaxonomyNode=(node:TaxonomyTreeModelNode,depth:number):React.ReactNode=>{
    ...
    return <Fragment key={node.id}><div className="taxonomy-sidebar-row" style={style}>{hasChildren?<Button variant="minimal" size="small" className="taxonomy-sidebar-toggle" icon={marker} aria-label={`${isExpanded?'Collapse':'Expand'} ${node.tag.name}`} onClick={()=>toggleTag(node.id)}/>:<span className="taxonomy-sidebar-toggle-spacer">{marker}</span>}<Button fill alignText="start" variant="minimal" className="nav-item taxonomy-sidebar-label" text={node.tag.name} title={node.tag.name} onClick={()=>app.setView({type:'taxonomy',id:node.tag.taxonomyId})}/></div>{isExpanded&&node.children.map((child)=>renderTaxonomyNode(child,depth+1))}</Fragment>
  }
  return <><aside className="sidebar"><div className="sidebar-navigation">
    <div className="sidebar-section"><SectionHeader title="Securities" menu={<PopoverNext content={<Menu className="sidebar-add-menu"><MenuItem icon="add" text="New security" onClick={onNewSecurity}/><MenuItem icon="folder-new" text="New watchlist" onClick={onNewWatchlist}/></Menu>} placement="bottom-end" animation="minimal" arrow={false} shouldReturnFocusOnClose={false}><Button variant="minimal" size="small" icon="add" aria-label="Add security or watchlist"/></PopoverNext>}/>
      <Button fill alignText="start" variant="minimal" icon="th-list" className={`nav-item securities-list-item all-securities-item ${app.view.type==='all-securities'?'active':''}`} text="All Securities" onClick={()=>app.setView({type:'all-securities'})} onContextMenu={openAllSecuritiesMenu}/>
  ...
  </div><PriceActivityIndicator/></aside>{renamingWatchlist&&<WatchlistForm watchlist={renamingWatchlist} onClose={()=>setRenamingWatchlist(undefined)}/>} {editingTaxonomy&&<TaxonomyForm taxonomy={editingTaxonomy} onClose={()=>setEditingTaxonomy(undefined)}/>} {deletingWatchlist&&<ConfirmDialog title="Delete watchlist" message={`Do you really want to delete ${deletingWatchlist.name}? Securities in this list will not be deleted.`} confirmLabel="Delete" onClose={()=>setDeletingWatchlist(undefined)} onConfirm={()=>app.deleteWatchlist(deletingWatchlist.id)}/>} {deletingTaxonomy&&<ConfirmDialog title="Delete taxonomy" message={`Do you really want to delete ${deletingTaxonomy.name} and all of its tags?`} confirmLabel="Delete" onClose={()=>setDeletingTaxonomy(undefined)} onConfirm={()=>app.deleteTaxonomy(deletingTaxonomy.id)}/>}</>
}
