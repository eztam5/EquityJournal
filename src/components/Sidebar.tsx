...
export function Sidebar({ onNewSecurity, onNewWatchlist, onNewTaxonomy, onNewTopic }: { onNewSecurity():void;onNewWatchlist():void;onNewTaxonomy():void;onNewTopic():void }) {
  ...
  const openWatchlistMenu=(event:MouseEvent,watchlist:Watchlist)=>{event.preventDefault();showContextMenu({targetOffset:{left:event.clientX,top:event.clientY},isDarkTheme:document.documentElement.classList.contains('bp6-dark'),content:<Menu><MenuItem icon="edit" text="Rename" onClick={()=>setRenamingWatchlist(watchlist)}/><MenuItem icon="trash" intent="danger" text="Delete" onClick={()=>setDeletingWatchlist(watchlist)}/></Menu>})}
  ...
  return <><aside className="sidebar">
    <div className="sidebar-section"><SectionHeader title="Securities" menu={<PopoverNext content={<Menu><MenuItem icon="add" text="New security" onClick={onNewSecurity}/><MenuItem icon="folder-new" text="New watchlist" onClick={onNewWatchlist}/></Menu>} placement="bottom-end" animation="minimal" arrow={false} shouldReturnFocusOnClose={false}><Button variant="minimal" size="small" icon="add" aria-label="Add security or watchlist"/></PopoverNext>}/>
      ...
      {app.watchlists.map((item)=><Button key={item.id} fill alignText="start" variant="minimal" icon="th-list" data-watchlist-id={item.id} className={`nav-item securities-list-item ${app.view.type==='watchlist'&&app.view.id===item.id?'active':''} ${dropTarget===item.id?'drop-target':''}`} text={item.name} onClick={()=>app.setView({type:'watchlist',id:item.id})} onContextMenu={(event)=>openWatchlistMenu(event,item)}/>)}
    </div>
