...
import { fetchSecurityLinkFavicon, securityLinkOrigin } from '../utils/securityLinkFavicons'
...
export function SettingsDialog({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const loadFavicon=async(template:SecurityLinkTemplate)=>{
    ...
    if(faviconPath)setLinkTemplates((templates)=>templates.map((current)=>current.id===template.id&&securityLinkOrigin(current.urlPattern)===origin?{...current,faviconPath}:current))
  }
