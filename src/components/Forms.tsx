...
export function SecurityForm({ security, onClose }: { security?: Security; onClose(): void }) {
  ...
  return <DraggableDialog title={security?'Edit security':'New security'} onClose={onClose}><form onSubmit={submit} className="form-grid"><FormGroup label="Symbol" labelFor="security-symbol"><InputGroup id="security-symbol" autoFocus maxLength={20} value={symbol} onChange={(e)=>setSymbol(e.target.value)} placeholder="For example, AAPL"/></FormGroup><FormGroup label="Currency" labelFor="security-currency"><InputGroup id="security-currency" maxLength={8} value={currency} onChange={(e)=>setCurrency(e.target.value)} placeholder="For example, USD"/></FormGroup><FormGroup label="Company name" labelFor="security-name"><InputGroup id="security-name" maxLength={160} value={name} onChange={(e)=>setName(e.target.value)} placeholder="For example, Apple Inc."/></FormGroup><ErrorText error={error}/><DialogActions><Button text="Cancel" onClick={onClose}/><Button type="submit" intent="primary" text="Save" disabled={!symbol.trim()||!currency.trim()||!name.trim()}/></DialogActions></form></DraggableDialog>
}
