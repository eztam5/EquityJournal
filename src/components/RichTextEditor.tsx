...
export function RichTextEditor({content,onChange,beforeNavigate}:{content:string;onChange(html:string):void;beforeNavigate?():void|boolean|Promise<void|boolean>}){
  ...
  return <div className="editor-shell"><div className="editor-toolbar">
    ...
    <ButtonGroup className="tool-group" variant="minimal"><Tool label="Bold" active={editor.isActive('bold')} onClick={()=>editor.chain().focus().toggleBold().run()}><Bold/></Tool><Tool label="Italic" active={editor.isActive('italic')} onClick={()=>editor.chain().focus().toggleItalic().run()}><Italic/></Tool><Tool label="Underline" active={editor.isActive('underline')} onClick={()=>editor.chain().focus().toggleUnderline().run()}><UnderlineIcon/></Tool><Tool label="Strikethrough" active={editor.isActive('strike')} onClick={()=>editor.chain().focus().toggleStrike().run()}><Strikethrough/></Tool>
      <PopoverNext content={textPalette} isOpen={colorMenu==='text'} onInteraction={(open)=>setColorMenu(open?'text':null)} placement="bottom-start" animation="minimal" arrow={false} shouldReturnFocusOnClose={false}><Tool label="Text color" onClick={()=>setColorMenu(colorMenu==='text'?null:'text')}><span className="letter-tool">A<i/></span></Tool></PopoverNext>
