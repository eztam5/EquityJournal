...
import { Button, Card, Tag as BlueprintTag } from '@blueprintjs/core'
...
export function SecurityDetailView({id}:{id:string}){
  ...
  return <main className="content page detail-page"><header className="security-heading"><h1>{security.symbol}</h1><p>{security.name}</p></header><Card className="info-card" elevation={0}><div><span>Symbol</span><strong>{security.symbol}</strong></div><div><span>Currency</span><strong>{security.currency}</strong></div><div><span>Company name</span><strong>{security.name}</strong></div></Card>
    <Card className="classification-card" elevation={0}><header><h2>Classification</h2><Button icon="add" text="Add tags" size="small" onClick={()=>setAssigning(true)}/></header>{grouped.length===0?<p className="muted">No tags assigned yet</p>:grouped.map(({taxonomy,tags})=><div className="assigned-group" key={taxonomy.id}><h3>{taxonomy.name}</h3><div>{tags.map((tag)=><BlueprintTag interactive round minimal onRemove={()=>remove(tag.id)} style={{borderColor:tag.color}} key={tag.id}>{tag.name}</BlueprintTag>)}</div></div>)}</Card>
    <Card className="notes-card" elevation={0}><header><h2>Research Notes</h2><span>{status}</span><Button icon="floppy-disk" text="Save" size="small" onClick={async()=>{await app.repository.saveNote(id,content);contentRef.current=content;setStatus('Saved')}}/></header><Suspense fallback={<div className="empty-state">Loading editor…</div>}><RichTextEditor key={id} content={content} onChange={setContent}/></Suspense></Card>{assigning&&<TagAssignmentDialog securityId={id} onClose={()=>setAssigning(false)} onSaved={loadAssignments}/>} 
  </main>
}
