...
import type { SecurityJournalEntry, Tag, Taxonomy } from '../domain/types'
...
export function SecurityDetailView({id}:{id:string}){
  ...
  useEffect(()=>{let active=true;setDocumentCount(0);app.repository.listSecurityDocuments(id).then((documents)=>{if(active)setDocumentCount(documents.length)}).catch(()=>{});return()=>{active=false}},[app.repository,id])
  useEffect(()=>{if(loadedNoteId!==id||content===contentRef.current)return;setStatus('Unsaved changes');const noteId=id,noteContent=content;const timer=setTimeout(async()=>{setStatus('Saving…');try{await app.repository.saveNote(noteId,noteContent);if(loadedIdRef.current!==noteId)return;contentRef.current=noteContent;setStatus('Saved')}catch(r){if(loadedIdRef.current===noteId)setStatus(r instanceof Error?r.message:String(r))}},1200);return()=>clearTimeout(timer)},[app.repository,content,id,loadedNoteId])
  ...
  const saveNote=async()=>{const noteId=id,noteContent=content;setStatus('Saving…');try{await app.repository.saveNote(noteId,noteContent);if(loadedIdRef.current!==noteId)return false;contentRef.current=noteContent;setStatus('Saved');return true}catch(reason){if(loadedIdRef.current===noteId)setStatus(reason instanceof Error?reason.message:String(reason));return false}}
  ...
  const openJournalEntry=(entry:SecurityJournalEntry)=>{setSelectedJournalEntryId(entry.id);setResearchTab('journal');requestAnimationFrame(()=>document.querySelector('.notes-card')?.scrollIntoView?.({behavior:'smooth',block:'start'}))}
  return <main className="content page detail-page"><PageHeader title={security.symbol} description={security.name} actions={<PageToolbarIconBar><PageToolbarIconButton icon="edit" label="Edit security" onClick={()=>setEditing(true)}/></PageToolbarIconBar>}/><Card className="content-panel info-card" elevation={0}><div><span>Symbol</span><strong>{security.symbol}</strong></div><div><span>Alternative ID</span><strong>{security.alternativeId||'—'}</strong></div><div><span>Currency</span><strong>{security.currency}</strong></div><div><span>Company name</span><strong>{security.name}</strong></div><div className="security-links"><span>Links</span>{links.length?<div>{links.map((link)=><Button key={link.id} variant="minimal" size="small" icon={<SecurityLinkIcon path={link.faviconPath}/>} aria-label={link.linkText} title={link.url} onClick={()=>openExternalUrl(link.url)}/>)}</div>:<strong>—</strong>}</div></Card>
    <SecurityPriceChart key={security.id} security={security} journalRevision={journalRevision} onJournalEntryClick={openJournalEntry}/>
    ...
    <Card className="content-panel notes-card" elevation={0}><header><h2>Research</h2>{researchTab==='thesis'&&<div className="research-save-controls"><Button icon={saveIcon} text={saveText} title={saveFailed?status:saveText} loading={status==='Saving…'} size="small" disabled={loadedNoteId!==id||status==='Saving…'} onClick={saveNote}/></div>}<ButtonGroup className="research-tabs" role="tablist"><Button role="tab" aria-selected={researchTab==='thesis'} active={researchTab==='thesis'} text="Current Thesis" onClick={()=>setResearchTab('thesis')}/><Button role="tab" aria-selected={researchTab==='journal'} active={researchTab==='journal'} text="Journal" onClick={()=>setResearchTab('journal')}/><Button role="tab" aria-selected={researchTab==='documents'} active={researchTab==='documents'} text={`Documents (${documentCount})`} onClick={()=>setResearchTab('documents')}/></ButtonGroup></header>{researchTab==='thesis'?(noteLoadError?<div className="empty-state">Could not load current thesis: {noteLoadError}</div>:loadedNoteId!==id?<div className="empty-state">Loading editor…</div>:<Suspense fallback={<div className="empty-state">Loading editor…</div>}><RichTextEditor key={id} content={content} onChange={setContent} beforeNavigate={saveNote} imageContext={{ownerType:'security',ownerId:id,contentType:'security-note',contentId:id}}/></Suspense>):researchTab==='journal'?<ResearchJournal key={id} securityId={id} selectedEntryId={selectedJournalEntryId} onSaved={()=>setJournalRevision((value)=>value+1)}/>:<SecurityDocuments key={id} securityId={id} onCountChange={setDocumentCount}/>}</Card>
